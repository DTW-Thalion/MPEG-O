# MPEG-O Version History

This document preserves the release-by-release feature narrative for MPEG-O. The
...
* **Format specification** — [`format-spec.md`](format-spec.md) documents every group, dataset, attribute, and compound type in enough detail for a conforming Python/Rust/Go reader. [`feature-flags.md`](feature-flags.md) is the feature string registry. Reference `.mpgo` fixtures under `objc/Tests/Fixtures/mpgo/` (generated by `objc/Tools/MakeFixtures`) provide canonical smoke-test files.
...
* **Thermo RAW stub (M29)** — `MPGOThermoRawReader` / `mpeg_o.importers.thermo_raw` stub; returns not-implemented error with SDK guidance. See [`vendor-formats.md`](vendor-formats.md).
...
* **Native Thermo RAW import (M35)** — `MPGOThermoRawReader` shells out to `ThermoRawFileParser` and ingests the resulting mzML. See [`vendor-formats.md`](vendor-formats.md).
...
through the generic `MPGOSpectrum` path with `@mpgo_class` attributes, so
