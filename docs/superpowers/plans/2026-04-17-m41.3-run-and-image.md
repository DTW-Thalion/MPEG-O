...
**Architecture:** ObjC normative. The headline work is making `AcquisitionRun` conform to `Indexable`, `Streamable`, and `Provenanceable` in both Python and Java (matching ObjC's `<TTIOIndexable, TTIOStreamable, TTIOProvenanceable, TTIOEncryptable>` conformances). `Encryptable` conformance is deferred to slice 41.5 where the encryption-manager subsystem lands. `SpectrumIndex` gains element-at accessors and range-query methods. `MSImage` gains the typed dataset-level fields ObjC inherits from `TTIOSpectralDataset` (title, isa_id, identifications, quantifications, provenance_records, tile_size) via composition rather than inheritance — Python/Java `SpectralDataset` is a file-handle wrapper whose lifecycle doesn't fit the MSImage subclass pattern; this divergence is a "known stylistic difference" per the spec.
...
Same as M41.1/M41.2:
- SoT: WSL-native `/home/toddw/TTI-O`.
...
- **`AcquisitionRun` `Encryptable` conformance** — moves to slice 41.5 (Protection). `encryptWithKey` / `decryptWithKey` / `accessPolicy` / `setAccessPolicy` in ObjC delegate to `TTIOEncryptionManager` via a persistence-context; building this in Python/Java requires the manager subsystem, which 41.5 handles.
- **`MSImage` extending `SpectralDataset`** — ObjC makes `TTIOMSImage` a `TTIOSpectralDataset` subclass to inherit "file container" semantics. Python and Java `SpectralDataset` are resource-holding file handles, not plain data classes; inheritance doesn't map cleanly. Handle via composition in this slice: MSImage gains the five dataset-level fields (`title`, `isa_investigation_id`, `identifications`, `quantifications`, `provenance_records`) as direct members. Document as a known stylistic difference in the api-review doc (slice 41.9).
...
- `objc/Source/Run/TTIOAcquisitionRun.h` — xref block
- `objc/Source/Run/TTIOInstrumentConfig.h` — xref block
- `objc/Source/Run/TTIOSpectrumIndex.h` — xref block
- `objc/Source/Image/TTIOMSImage.h` — xref block

### Python
- Modify: `python/src/ttio/acquisition_run.py` — declare Indexable/Streamable/Provenanceable conformance; add missing methods
- Modify: `python/src/ttio/instrument_config.py` — NumPy docstring + xref
- Modify: the `SpectrumIndex` class inside `acquisition_run.py` — add element-at accessors + range-query methods; docstring
- Modify: `python/src/ttio/ms_image.py` — add 5 dataset-level fields + `tile_size`; NumPy docstring
- Modify: `python/tests/test_value_classes.py` — parity tests

### Java
- Modify: `java/src/main/java/com/dtwthalion/ttio/AcquisitionRun.java` — `implements Indexable<Spectrum>, Streamable<Spectrum>, Provenanceable, AutoCloseable`; add missing methods; Javadoc
- Modify: `java/src/main/java/com/dtwthalion/ttio/InstrumentConfig.java` — Javadoc
- Modify: `java/src/main/java/com/dtwthalion/ttio/SpectrumIndex.java` — add missing `*At(i)` methods + range queries; Javadoc
- Modify: `java/src/main/java/com/dtwthalion/ttio/MSImage.java` — add 5 dataset-level fields + `tileSize`; Javadoc
- Modify: `java/src/test/java/com/dtwthalion/ttio/ValueClassesTest.java` — parity tests
...
**Goal:** Both languages gain element-at accessors and range-query methods matching ObjC's `TTIOSpectrumIndex` surface.
...
**Files:**
- Modify: `python/src/ttio/acquisition_run.py` — the `SpectrumIndex` dataclass at the top
- Modify: `java/src/main/java/com/dtwthalion/ttio/SpectrumIndex.java`
...
**ObjC** — append xref block to `TTIOSpectrumIndex.h` class-level `/** ... */`:
```
 *
 * API status: Stable.
 *
...
class InstrumentConfig:
    ...
    ``com.dtwthalion.tio.InstrumentConfig``.
    """
...
**ObjC** — append xref block to `TTIOInstrumentConfig.h`.
...
def test_acquisition_run_conforms_to_protocols():
    ...
    from ttio.acquisition_run import AcquisitionRun, SpectrumIndex
...
```java
    ...
    void acquisitionRunImplementsProtocols() throws Exception {
        ...
        // Indexable
        assertTrue(run instanceof com.dtwthalion.tio.protocols.Indexable);
        assertEquals(2, run.count());
        assertNotNull(run.objectAtIndex(0));

        // Streamable
        assertTrue(run instanceof com.dtwthalion.tio.protocols.Streamable);
        ...
        // Provenanceable
        assertTrue(run instanceof com.dtwthalion.tio.protocols.Provenanceable);
        assertEquals(java.util.List.of(), run.provenanceChain());
        assertEquals(java.util.List.of(), run.inputEntities());
    }
```

**ObjC** — append xref block to `TTIOAcquisitionRun.h`.
...
**Goal:** Python + Java `MSImage` gain the five dataset-level fields ObjC inherits from `TTIOSpectralDataset`: `title`, `isa_investigation_id`, `identifications`, `quantifications`, `provenance_records`. Python adds `tile_size` (already present). Java adds `tile_size`. Document as composition rather than inheritance.
...
class MSImage:
    ...
    In Objective-C, ``TTIOMSImage`` inherits from ``TTIOSpectralDataset``
...
import com.dtwthalion.tio.hdf5.Hdf5Dataset;
import com.dtwthalion.tio.hdf5.Hdf5Group;
 ...
 *
 * <p>In Objective-C, {@code TTIOMSImage} inherits from
 * {@code TTIOSpectralDataset}. In Java, {@code SpectralDataset} is a
 ...
 * <p><b>API status:</b> Stable.</p>
 *
 * <p><b>Cross-language equivalents:</b> Objective-C {@code TTIOMSImage},
 * Python {@code ttio.ms_image.MSImage}.</p>
 *
 * @since 0.6
 */
...
```bash
wsl -d Ubuntu -- bash -c 'cd ~/TTI-O/java && grep -rn --include="*.java" "new MSImage(" src/'
```
...
```bash
wsl -d Ubuntu -- bash -c 'cd ~/TTI-O/python && grep -rn --include="*.py" "MSImage(" src/ tests/'
```
The new fields default to empty; existing kwargs-based constructions keep working.

**ObjC** — append xref block to `TTIOMSImage.h`.
...
- `objc/Source/Run/TTIOAcquisitionRun.h`
- `objc/Source/Run/TTIOInstrumentConfig.h`
- `objc/Source/Run/TTIOSpectrumIndex.h`
- `objc/Source/Image/TTIOMSImage.h`
- `python/src/ttio/acquisition_run.py`
- `python/src/ttio/instrument_config.py`
- `python/src/ttio/ms_image.py`
- `python/tests/test_value_classes.py`
- `java/src/main/java/com/dtwthalion/ttio/AcquisitionRun.java`
- `java/src/main/java/com/dtwthalion/ttio/InstrumentConfig.java`
- `java/src/main/java/com/dtwthalion/ttio/SpectrumIndex.java`
- `java/src/main/java/com/dtwthalion/ttio/MSImage.java`
