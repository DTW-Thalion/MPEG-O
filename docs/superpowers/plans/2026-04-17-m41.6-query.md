...
**Python** — new `python/src/mpeg_o/query.py`:
...
**Java** — new `java/src/main/java/com/dtwthalion/mpgo/Query.java`:
...
**ObjC** — append xref block to `MPGOQuery.h`.
...
**Python** — new `python/src/mpeg_o/stream_reader.py`:
...
**Java** — new `java/src/main/java/com/dtwthalion/mpgo/StreamReader.java`:
...
**ObjC** — append xref block to `MPGOStreamReader.h`.

**Parity tests:** construct a minimal in-memory `.mpgo`-like file, open a StreamReader, iterate through it.
...
Incremental append-and-flush writer that regenerates the target file on each flush. ObjC's implementation accumulates spectra in memory and on `flushWithError:` writes a new `.mpgo` with every spectrum so far. Python + Java mirror this pattern.

**Python** — new `python/src/mpeg_o/stream_writer.py`:
...
**Java** — new `java/src/main/java/com/dtwthalion/mpgo/StreamWriter.java`:
...
**ObjC** — append xref block to `MPGOStreamWriter.h`.
