...
| **M98 encrypted-graph exchange** (3×3 encryptor × decryptor via the PerAU CLIs' `decrypt-in-place`, per-AU flag stripped, one AU per segment record, byte-exact GFA re-emission) | `=` | `=` | `=` | `=` | `python/tests/validation/test_m98_gfa_matrix.py` (drives `TtioPerAU` + Java `PerAUCli`) |
| **M99 blocks_v1 per-AU exchange** (3×3 encryptor × decryptor over stream-written blocks_v1 runs — cross-chromosome mates, zero-length reads, embedded-reference REF_DIFF_V2 — flag stripped, one AU per read, byte-identical channel blobs + block index after restore) | `=` | `=` | `=` | `=` | `python/tests/validation/test_m99_blocks_v1_matrix.py` (drives `TtioPerAU` + Java `PerAUCli`) |
| **M99.1 blocks_v1 transport exchange** (3×3 sender × receiver via the PerAU CLIs' `send`/`recv` over a per-AU-encrypted blocks_v1 run — v0.12 sidecar packets, decrypt-in-place on the received container byte-identical, sidecar channels + index arrays + name tables verbatim) | `=` | `=` | `=` | `=` | `python/tests/validation/test_m99_blocks_v1_matrix.py` (drives `TtioPerAU` + Java `PerAUCli`) |
| **M100 writable reopen → references embed** (3×3 writer × reader over `/study/references/`; each writer = minimal write, writable reopen, write-to-dataset) | `=` | `=` | `=` | `=` | `python/tests/conformance/test_references_xlang.py` (drives `TtioRefXLangWriter` + Java `RefXLangWriter`) |
...
| Per-AU encrypt/decrypt/decrypt-in-place/transcode/send/recv | `python -m ttio.tools.per_au_cli` | `objc/Tools/obj/TtioPerAU` | `global.thalion.ttio.tools.PerAUCli` |
