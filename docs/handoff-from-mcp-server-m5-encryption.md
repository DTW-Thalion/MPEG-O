# Encryption API bugs surfaced by MPEG-O-MCP-Server M5 work

## Context

The downstream **MPEG-O-MCP-Server** (github.com/DTW-Thalion/MPEG-O-MCP-Server, currently at `v0.4.0.dev0`) is about to add keyring-backed encryption support in its M5 milestone. It pins `mpeg-o @ git+https://github.com/DTW-Thalion/MPEG-O.git@v1.0.0#subdirectory=python`. A smoke test against the public `SpectralDataset.encrypt_with_key` / `decrypt_with_key` API on **v1.0.0** surfaced two blocking issues that need to be fixed in the MPEG-O reference implementation (ObjC canonical) and mirrored in Java + Python per the project's 3-language parity rule.
...
from mpeg_o import SpectralDataset, AcquisitionMode, WrittenRun
...
That's effectively forcing downstream callers to re-implement MPEG-O's own HDF5 layout knowledge. After encryption the dataset also *loses* the `intensity` signal array — `spec.intensity_array` raises `KeyError: "no such signal array 'intensity'; have ['mz']"` — so there's no fallback path through the normal spectrum read API.
...
Option 1 is the most aligned with how the rest of MPEG-O is used.
...
- **ObjC (canonical reference impl)** — start here. `MPGOSpectralDataset -encryptWithKey:level:` and `-decryptWithKey:` are the source of truth.
...
The MCP server is built from `//wsl.localhost/Ubuntu/home/toddw/MPEG-O-MCP-Server` (WSL) and pushed via Windows git (WSL HTTPS auth hangs). Once the MPEG-O tag lands, the downstream pin in `pyproject.toml` gets bumped and M5 resumes with encryption in scope.
