...
`global.thalion.ttio.transport` / `.protection`.
...
`global.thalion.ttio.protection.PerAU*` /
...
uses PascalCase in `global.thalion.ttio.{RamanSpectrum,IRSpectrum,
RamanImage,IRImage}`.
...
- **JCAMP-DX 5.01 AFFN bridge** — `TTIOJcampDxReader` /
  ...
  and `global.thalion.ttio.{importers,exporters}.JcampDx*`). All
