...
def _lz4_filter_kwargs() -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "install with 'pip install ttio[codecs]'"
        ) from exc
...
def write_signal_channel(
    ...
    *,
    ...
    - ``"gzip"`` (default, zlib) — matches the ObjC
      ``TTIOCompressionZlib`` default with ``compression_level``
...
FEATURES_ATTR = "ttio_features"
VERSION_ATTR = "ttio_format_version"
LEGACY_VERSION_ATTR = "ttio_version"


def write_feature_flags(root: _IOTarget, version: str, features: Iterable[str]) -> None:
    """Write ``@ttio_format_version`` and ``@ttio_features`` on ``/``."""
...
def read_feature_flags(root: _IOTarget) -> tuple[str, list[str]]:
    ...
    Returns ``("1.0.0", [])`` for files that carry only ``@ttio_version``.
    """
...
def is_legacy_v1(root: _IOTarget) -> bool:
    """A file is v0.1-legacy when it has no ``@ttio_features`` attribute."""
