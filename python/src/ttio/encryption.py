...
def _aesgcm():  # type: ignore[no-untyped-def]
    try:
        ...
        raise RuntimeError(
            "ttio.encryption requires the 'cryptography' optional dependency; "
            "install with 'pip install ttio[crypto]'"
        ) from exc
    return AESGCM
...
def encrypt_intensity_channel_in_run(
    ...
    Matches ObjC
    ``+[TTIOEncryptionManager encryptIntensityChannelInRun:atFilePath:withKey:error:]``.

    Opens the .tio file read-write, locates
...
def decrypt_intensity_channel_in_run(
    ...
    Matches ObjC
    ``+[TTIOEncryptionManager decryptIntensityChannelInRun:atFilePath:withKey:error:]``.
...
def decrypt_intensity_channel_in_run_in_place(
    ...
    .tio file read-write, decrypts ``intensity_values_encrypted`` for
...
def decrypt_intensity_channel_in_group(
    ...
    :class:`~ttio.spectral_dataset.SpectralDataset`) and cannot reopen
