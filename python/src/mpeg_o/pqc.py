...
rest of :mod:`mpeg_o` a stable surface for FIPS 203 (ML-KEM-1024) key
...
:class:`PQCUnavailableError` at call time — importing :mod:`mpeg_o.pqc`
...
by FIPS 203 and FIPS 204; see :mod:`mpeg_o.cipher_suite` for the
catalog values.

Cross-language equivalents
--------------------------
* Objective-C: ``MPGOCipherSuite`` (same liboqs library, C API)
...
def _oqs():  # type: ignore[no-untyped-def]
    try:
        ...
        raise PQCUnavailableError(
            "mpeg_o.pqc requires the optional 'liboqs-python' dependency — "
            "install with: pip install 'mpeg-o[pqc]'"
        ) from exc
    return oqs
