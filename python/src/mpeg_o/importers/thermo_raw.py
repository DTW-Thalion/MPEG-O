...
``.raw`` → mzML, then parses the mzML via :mod:`mpeg_o.importers.mzml`.
No proprietary code ships with MPEG-O.
...
def read(path: str | Path, *,
    ...
    with tempfile.TemporaryDirectory(prefix="mpgo_thermo_") as tmp:
        out_dir = Path(tmp)
        cmd = list(cmd_prefix) + [
            "-i", str(raw),
            "-o", str(out_dir),
