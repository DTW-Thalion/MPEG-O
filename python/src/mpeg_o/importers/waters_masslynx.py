...
and writes mzML; MPEG-O then parses the mzML via
:mod:`mpeg_o.importers.mzml`. No proprietary code ships with MPEG-O.
...
--------------------------
Objective-C: ``MPGOWatersMassLynxReader``
...
def read(raw_dir: str | Path, *, converter: str | None = None) -> ImportResult:
    ...
    with tempfile.TemporaryDirectory(prefix="mpgo_masslynx_") as tmp:
        out_dir = Path(tmp)
        cmd = list(cmd_prefix) + [
            "-i", str(src),
            "-o", str(out_dir),
