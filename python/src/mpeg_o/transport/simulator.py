...
Produces synthetic MPEG-O transport packets that mimic an LC-MS
...
``com.dtwthalion.mpgo.transport.AcquisitionSimulator``, ObjC
``MPGOAcquisitionSimulator``.
"""
...
def _generate_au(
    ...
    au = AccessUnit(
        spectrum_class=_SPECTRUM_CLASS_TO_WIRE["MPGOMassSpectrum"],
        acquisition_mode=int(AcquisitionMode.MS1_DDA),
        ms_level=ms_level,
