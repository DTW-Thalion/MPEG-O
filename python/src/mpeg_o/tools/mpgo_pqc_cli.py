"""``mpgo-pqc`` — Python post-quantum crypto CLI (M75).

Python equivalent of the Objective-C ``MpgoPQCTool`` (``objc/Tools/
MpgoPQCTool.m``) and Java ``com.dtwthalion.mpgo.tools.PQCTool``. The
...
Subcommands::
    ...
    mpgo-pqc sig-keygen       PK_OUT  SK_OUT
    mpgo-pqc sig-sign         SK_IN   MSG_IN  SIG_OUT
    mpgo-pqc sig-verify       PK_IN   MSG_IN  SIG_IN       (exit 0/1/2)
    mpgo-pqc kem-keygen       PK_OUT  SK_OUT
    mpgo-pqc kem-encaps       PK_IN   CT_OUT  SS_OUT
    mpgo-pqc kem-decaps       SK_IN   CT_IN   SS_OUT
    mpgo-pqc hdf5-sign        FILE    DS_PATH  SK_IN
    mpgo-pqc hdf5-verify      FILE    DS_PATH  PK_IN       (exit 0/1/2)
    mpgo-pqc provider-sign    URL     DS_PATH  SK_IN
    mpgo-pqc provider-verify  URL     DS_PATH  PK_IN       (exit 0/1/2)
...
Requires the ``[pqc]`` optional extra (``pip install 'mpeg-o[pqc]'``).
"""
...
USAGE = (
    "usage: mpgo-pqc <subcommand> [args...]\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        sys.stderr.write(f"mpgo-pqc {sub} failed: {e}\n")
        return 2
