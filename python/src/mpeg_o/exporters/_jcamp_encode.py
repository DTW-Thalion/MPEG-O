...
Mirror-image of :mod:`mpeg_o.importers._jcamp_decode`. Takes a
...
(``com.dtwthalion.mpgo.exporters.JcampDxWriter``) and Objective-C
(``MPGOJcampDxWriter``) writers — byte-for-byte identical output is
...
``%.10g`` so a non-MPGO JCAMP-DX reader can audit it; the matching
...
--------------------------
Objective-C: ``MPGOJcampDxWriter+Compress.m`` · Java:
``com.dtwthalion.mpgo.exporters.JcampDxWriter#encodeXYData``.
"""
...
def encode_xydata(
    ...
    *,
    ...
    fast path lives in :mod:`mpeg_o.exporters.jcamp_dx` and is not
    routed through here.
