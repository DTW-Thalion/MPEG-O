...
--------------------------
Objective-C: ``MPGOMzTabWriter`` · Java:
``com.dtwthalion.mpgo.exporters.MzTabWriter``
...
def dataset_to_bytes(
    ...
    * ``version="2.0.0-M"`` — metabolomics. Emit SML rows that carry
    ...
    if version == "1.0":
        lines.append("MTD\tmzTab-ID\tmpgo-export")
    if title:
        lines.append(f"MTD\ttitle\t{_escape_tsv(title)}")
    if description:
        lines.append(f"MTD\tdescription\t{_escape_tsv(description)}")
    lines.append("MTD\tsoftware[1]\t[MS, MS:1000799, custom unreleased software tool, mpeg-o]")
