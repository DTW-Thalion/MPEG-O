...
* ``@_mpgo_kind = "compound"``
* ``@_mpgo_schema = <JSON-list-of-{name, kind}>``
* ``@_mpgo_rows = <JSON-list-of-row-dicts>``
...
try:
    ...
    raise ImportError(
        "mpeg_o.providers.zarr requires the optional 'zarr' dependency. "
        "Install with `pip install 'mpeg-o[zarr]'` or `pip install zarr`."
    ) from e
...
_COMPOUND_KIND_ATTR = "_mpgo_kind"
_COMPOUND_KIND_VALUE = "compound"
_COMPOUND_SCHEMA_ATTR = "_mpgo_schema"
_COMPOUND_ROWS_ATTR = "_mpgo_rows"
_COMPOUND_COUNT_ATTR = "_mpgo_count"
_ND_SHAPE_ATTR = "_mpgo_nd_shape"  # preserved for parity; zarr knows shape
...
class _ZarrCompoundDataset(StorageDataset):
    ...
    def has_attribute(self, name: str) -> bool:
        return name in self._group.attrs and not name.startswith("_mpgo_")
    ...
    def attribute_names(self) -> list[str]:
        return [k for k in self._group.attrs.keys() if not k.startswith("_mpgo_")]
...
def _coerce_attr_for_json(value: Any) -> Any:
    ...
    :mod:`base64` is a v0.8 concern — MPEG-O attrs are str / int /
    float today."""
