"""M75 Python CLI parity — smoke tests for mpgo-sign, mpgo-verify, mpgo-pqc.

Covers:

1. ``[project.scripts]`` entry points are declared and resolve to the
   ``main`` callable in each CLI module (no subprocess needed).
2. ``mpgo-sign`` + ``mpgo-verify`` round-trip via ``python -m`` on a
   minimal ``.mpgo`` fixture (no PATH lookup, always portable).
3. ``mpgo-verify`` reports ``INVALID`` on a tampered signature and
   ``NOT_SIGNED`` on an unsigned dataset.
4. ``mpgo-pqc`` sig-keygen + sig-sign + sig-verify round-trip, gated
   on the ``[pqc]`` extra being installed.

The console-script entry points exercised here are the Python
counterparts of the Objective-C ``MpgoVerify`` / ``MpgoSign`` /
``MpgoPQCTool`` and Java ``MpgoVerify`` / ``PQCTool`` binaries. M75
...
def _make_fixture(path: Path) -> None:
    SpectralDataset.write_minimal(
        path, title="m75", isa_investigation_id="MPGO:m75",
...
@pytest.mark.parametrize("name,target", [
    ("mpgo-sign",   "mpeg_o.tools.mpgo_sign_cli:main"),
    ("mpgo-verify", "mpeg_o.tools.mpgo_verify_cli:main"),
    ("mpgo-pqc",    "mpeg_o.tools.mpgo_pqc_cli:main"),
])
...
# ─── mpgo-sign + mpgo-verify round-trip ──────────────────────────────────


def test_sign_verify_roundtrip(tmp_path: Path) -> None:
    fixture = tmp_path / "m75.mpgo"
    _make_fixture(fixture)

    sign = _run(
        "mpeg_o.tools.mpgo_sign_cli",
...
def test_verify_invalid_on_wrong_key(tmp_path: Path) -> None:
    fixture = tmp_path / "m75_invalid.mpgo"
    _make_fixture(fixture)
    _run("mpeg_o.tools.mpgo_sign_cli",
...
def test_verify_not_signed(tmp_path: Path) -> None:
    fixture = tmp_path / "m75_unsigned.mpgo"
    _make_fixture(fixture)
    verify = _run(
        "mpeg_o.tools.mpgo_verify_cli",
        str(fixture), DATASET_PATH, FIXTURE_KEY_HEX,
...
def test_sign_rejects_non_hex_key(tmp_path: Path) -> None:
    fixture = tmp_path / "m75_bad_key.mpgo"
    _make_fixture(fixture)
    proc = _run(
        "mpeg_o.tools.mpgo_sign_cli",
...
def test_sign_rejects_missing_dataset(tmp_path: Path) -> None:
    fixture = tmp_path / "m75_no_ds.mpgo"
    _make_fixture(fixture)
    proc = _run(
        "mpeg_o.tools.mpgo_sign_cli",
...
# ─── mpgo-pqc round-trip (gated on [pqc] extra) ──────────────────────────
...
def test_pqc_hdf5_sign_verify(tmp_path: Path) -> None:
    ...
    with h5py.File(fixture, "r") as f:
        stored = f[DATASET_PATH].attrs["mpgo_signature"]
...
def test_pqc_unknown_subcommand_exits_2() -> None:
    proc = _run("mpeg_o.tools.mpgo_pqc_cli", "bogus-subcmd")
