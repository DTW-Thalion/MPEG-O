"""Layer 4 — chr22 ratio gate per spec §9.4.

Encodes the chr22 NA12878 lean+mapped corpus end-to-end via
SpectralDataset.write_minimal with v2 default ON vs opt-out.
Asserts savings >= 2 MB; logs file sizes.

The hard gate is the formal acceptance criterion for REF_DIFF v2
shipping in v1.8.  Design target was ~4.5 MB (sequences channel:
11.34 MB -> 6.80 MB per T6 oracle); gate is 2 MB (conservative).

Run with::

    TTIO_RANS_LIB_PATH=/home/toddw/TTI-O/native/_build/libttio_rans.so \\
        python -m pytest python/tests/integration/test_ref_diff_v2_compression_gate.py \\
        -m integration -v -s
"""
from __future__ import annotations

from pathlib import Path

import pytest

from ttio.codecs import ref_diff_v2 as rdv2

REPO_ROOT = Path(__file__).resolve().parents[3]
CHR22_BAM = REPO_ROOT / "data" / "genomic" / "na12878" / "na12878.chr22.lean.mapped.bam"
CHR22_REF = REPO_ROOT / "data" / "genomic" / "reference" / "hs37.chr22.fa"

pytestmark = [
    pytest.mark.integration,
    pytest.mark.skipif(
        not rdv2.HAVE_NATIVE_LIB,
        reason="needs libttio_rans (set TTIO_RANS_LIB_PATH)",
    ),
    pytest.mark.skipif(
        not CHR22_BAM.exists(),
        reason=f"chr22 corpus not on disk: {CHR22_BAM}",
    ),
    pytest.mark.skipif(
        not CHR22_REF.exists(),
        reason=f"chr22 reference FASTA not on disk: {CHR22_REF}",
    ),
]


def _load_reference_chroms(ref_fasta: Path, chroms_used: set) -> dict:
    """Read only the chromosomes touched by the run from the FASTA file."""
    out: dict[str, bytes] = {}
    if not chroms_used:
        return out
    target_set = {c.encode("ascii") for c in chroms_used}
    current = None
    buf = bytearray()
    with ref_fasta.open("rb") as fh:
        for line in fh:
            if line.startswith(b">"):
                if current is not None:
                    out[current.decode("ascii")] = bytes(buf).upper()
                hdr = line[1:].split()[0] if len(line) > 1 else b""
                current = hdr if hdr in target_set else None
                buf.clear()
            elif current is not None:
                buf.extend(line.strip())
        if current is not None:
            out[current.decode("ascii")] = bytes(buf).upper()
    return out


def _encode_chr22(out_path: Path, *, opt_disable_v2: bool) -> None:
    """Build a v1.8 .tio file from the chr22 BAM with REF_DIFF v2 on/off.

    Uses the same BamReader + SpectralDataset.write_minimal pipeline
    as the benchmark harness (``tools/benchmarks/formats.py``), but
    without the FQZCOMP / NAME_TOKENIZED overrides — we want to isolate
    the sequences channel savings cleanly.  The only variable is
    ``opt_disable_ref_diff_v2``.
    """
    from ttio.importers.bam import BamReader
    from ttio import SpectralDataset

    run = BamReader(str(CHR22_BAM)).to_genomic_run(name="chr22")
    run.opt_disable_ref_diff_v2 = opt_disable_v2

    # Load the reference chromosome sequences — required for REF_DIFF v1 and v2.
    # Without this, both paths fall back to BASE_PACK and produce identical output.
    chroms_used = set(run.chromosomes) - {"*"}
    chrom_seqs = _load_reference_chroms(CHR22_REF, chroms_used)
    run.reference_chrom_seqs = chrom_seqs if chrom_seqs else None

    SpectralDataset.write_minimal(
        path=str(out_path),
        title="ref_diff_v2_gate:chr22",
        isa_investigation_id="TTIO:gate:ref_diff_v2",
        runs={},
        genomic_runs={"chr22": run},
    )


def test_chr22_ref_diff_v2_savings_ge_2mb(tmp_path: Path) -> None:
    """chr22 NA12878 lean+mapped: REF_DIFF v2 saves >= 2 MB vs v1.

    Hard gate: if this test fails the REF_DIFF v2 codec has
    regressed and MUST NOT ship.  The 2 MB floor is conservative;
    the design target is ~4.5 MB (v1 sequences ~11.34 MB ->
    v2 ~6.80 MB for chr22 1,766,433 records).
    """
    out_v1 = tmp_path / "chr22_v1.tio"
    out_v2 = tmp_path / "chr22_v2.tio"

    _encode_chr22(out_v1, opt_disable_v2=True)
    _encode_chr22(out_v2, opt_disable_v2=False)

    v1_size = out_v1.stat().st_size
    v2_size = out_v2.stat().st_size
    savings  = v1_size - v2_size

    MB = 1024 * 1024
    print(
        f"\nchr22 NA12878 lean+mapped (n=1,766,433 records):\n"
        f"  v1 (opt-out)  size : {v1_size:>15,d} bytes  "
        f"({v1_size / MB:8.3f} MB)\n"
        f"  v2 (default)  size : {v2_size:>15,d} bytes  "
        f"({v2_size / MB:8.3f} MB)\n"
        f"  savings             : {savings:>15,d} bytes  "
        f"({savings / MB:8.3f} MB)\n"
        f"  pct delta           : {100.0 * savings / v1_size:.2f}%"
    )

    assert savings >= 2 * MB, (
        f"chr22 savings {savings / MB:.3f} MB < 2 MB hard gate — "
        f"REF_DIFF v2 regressed or native lib not loaded correctly."
    )
