"""M87 cross-language conformance harness for the SAM/BAM importer.

Drives the three language implementations via subprocess and
confirms each emits byte-identical canonical JSON for the same
input BAM. Any divergence in SAM parsing, RNEXT expansion,
1-based position handling, or canonical-JSON serialisation
trips the test.
...
- ``samtools`` is missing from PATH (M87 dispatch requires it).
...
FIXTURE = REPO_ROOT / "python" / "tests" / "fixtures" / "genomic" / "m87_test.bam"
...
def _python_dump() -> str:
    """Run the Python bam_dump CLI and return canonical JSON."""
    result = subprocess.run(
...
def _objc_dump() -> str:
    """Run the ObjC TtioBamDump CLI and return canonical JSON."""
...
def _java_dump() -> str:
    """Run the Java BamDump CLI via Maven exec and return canonical JSON."""
    ...
    # Maven exec:java prints to stdout but may include a trailing newline beyond
    # the JSON. Strip Maven banner if present.
    return result.stdout


def test_python_dump_works():
    """Sanity: Python CLI produces non-empty canonical JSON."""
    out = _python_dump()
    assert out.startswith("{")
    assert '"read_count": 10' in out
    assert '"sample_name": "M87_TEST_SAMPLE"' in out


def test_objc_matches_python_byte_exact():
    """ObjC TtioBamDump output is byte-identical to Python bam_dump."""
    py = _python_dump()
    oc = _objc_dump()
...
def test_java_matches_python_byte_exact():
    """Java BamDump output is byte-identical to Python bam_dump."""
    py = _python_dump()
    ja = _java_dump()
