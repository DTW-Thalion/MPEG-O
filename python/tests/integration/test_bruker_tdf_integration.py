"""Bruker TDF .d → .tio integration test (v0.9 M58).
...
is available, the produced .tio must carry the inv_ion_mobility
...
def test_bruker_d_to_ttio_preserves_ion_mobility(tmp_path: Path) -> None:
    """Bruker .d → .tio round-trip preserves frame count and the
    ...
    via the ``TTIO_BRUKER_TDF_FIXTURE`` environment variable.
    """
    fixture_env = os.environ.get("TTIO_BRUKER_TDF_FIXTURE")
    if not fixture_env:
        pytest.skip("TTIO_BRUKER_TDF_FIXTURE not set; no real Bruker .d available")
    d = Path(fixture_env)
    if not d.is_dir():
        pytest.skip(f"TTIO_BRUKER_TDF_FIXTURE={d} not a directory")

    out = tmp_path / "tims.tio"
    bruker_tdf.read(d, out)
    with SpectralDataset.open(out) as ds:
        run = ds.ms_runs.get("tims_ms1")
        assert run is not None, ".tio missing tims_ms1 run"
        assert len(run) > 0
        first = run[0]
