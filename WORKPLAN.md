...
- Wire rANS, base-pack, quality, and name-tokenizer into the genomic
  signal channel write/read paths.
- `@_ttio_codec` attribute on coded channels (provider-agnostic data
  transform, not HDF5 filter).
...
- All three languages + cross-language round-trip.
