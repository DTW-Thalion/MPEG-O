...
| 93 | Per-AU encryption of a `blocks_v1` genomic run (M99) walks block by block: per-block codec decode from the block index, one AU per read with global AU numbering and global plaintext offsets, extendable segments tables appended per block. Decrypt-in-place re-encodes each block through the block writer under the sticky qualities discipline (block 0 auto-tunes, the winner read back from the encoded stream pins the rest) and restores the channel blobs byte-identically; the block index is never rewritten. | The block policy already bounds a block's decoded size, so streaming falls out of the layout; byte-identical restore keeps the index valid without rewriting it, and the sticky pin is recoverable from the first re-encoded blob, so no restore metadata is stored. |
| 94 | The M99 encrypt walker re-encodes and byte-compares every block BEFORE deleting the plaintext channels, and refuses the run when any blob is not reproducible; a REF_DIFF_V2 run additionally requires its reference embedded in `/study/references/`. | Writer policy the file does not persist (`ref_diff_slice_bytes`, the qualities-V5 opt-out) would break restore reproducibility; verifying up front turns silent unrestorable files into a clear refusal, and an external REF_PATH reference cannot be re-verified once the original blob is gone. |
| 95 | The v1.0 encrypted transport stream does not carry `blocks_v1` genomic runs; all three senders refuse them (M99). | The stream's receiver rebuilds the legacy genomic shape only — without the block index and the coded read_names/cigars/mate blobs the received container cannot be restored; carrying them is a transport-spec extension with its own Phase 0. |
