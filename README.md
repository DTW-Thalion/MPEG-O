...
* **REF_DIFF** (M93, v1.2) — reference-based sequence-diff codec for genomic `sequences` channels (codec id `9`). Slice-based wire format with embedded reference at `/study/references/<reference_uri>/`; closes ~80% of the chr22 sequence-channel compression gap to CRAM 3.1. See [`docs/codecs/ref_diff.md`](docs/codecs/ref_diff.md).
* **FQZCOMP_NX16_Z** (M94.Z, v1.2) — CRAM-mimic lossless quality codec (codec id `12`). Static-per-block frequency tables, 16-bit renormalisation, T = 4096 fixed (`T \| b·L` exactly — byte-pairing invariant). Default for v1.5 quality channels; ~22× faster than the legacy M94 v1 (which was removed in Phase 10, 2026-04-30). See [`docs/codecs/fqzcomp_nx16_z.md`](docs/codecs/fqzcomp_nx16_z.md).
* **DELTA_RANS_ORDER0** (M95, v1.2) — delta + zigzag + LEB128 + rANS order-0 wrapper for sorted-ascending integer channels (codec id `11`). Auto-default for `positions` on v1.5 genomic runs. See [`docs/codecs/delta_rans.md`](docs/codecs/delta_rans.md).
* **`libttio_rans`** (Phase 10, 2026-04-30) — optional native C library at `native/` providing AVX2/SSE4.1/scalar SIMD-dispatched rANS kernels (~510 MiB/s encode, ~605 MiB/s decode on AVX2 hosts). Consumed by Python (ctypes), Java (JNI), and ObjC (direct linkage). Powers M94.Z V2 wire format (version byte = 2) when callers opt in via `prefer_native` or `TTIO_M94Z_USE_NATIVE=1`; the default V1 path runs through Cython / pure-Java / pure-ObjC and is unchanged. Phase 11 (2026-05-01) added `ttio_rans_decode_block_m94z` — a V2-decode entry that bakes the M94.Z context formula directly into native code, eliminating the per-symbol cross-language callback overhead. See [`docs/native-rans-library.md`](docs/native-rans-library.md).
