...
{
    return [NSString stringWithFormat:@"/tmp/ttio_test_run_%d_%@.tio",
            (int)getpid(), suffix];
}
...
static TTIOMassSpectrum *makeSpectrum(NSUInteger k)
{
    ...
    TTIOSignalArray *mzA = float64Array(mz, N);
    TTIOSignalArray *inA = float64Array(in, N);
    free(mz); free(in);
    ...
    TTIOMassSpectrum *s =
        [[TTIOMassSpectrum alloc] initWithMzArray:mzA
                                   intensityArray:inA
                                          msLevel:1 + (k % 2)
                                         polarity:(k % 4 == 0 ? TTIOPolarityNegative : TTIOPolarityPositive)
                                       scanWindow:nil
                                    indexPosition:k
...
{
    // ---- TTIOInstrumentConfig round-trip ----
    {
        ...
        PASS([cfg writeToGroup:[f rootGroup] error:&err],
             "TTIOInstrumentConfig writes");
        [f close];

        TTIOHDF5File *g = [TTIOHDF5File openReadOnlyAtPath:path error:&err];
        TTIOInstrumentConfig *back =
            [TTIOInstrumentConfig readFromGroup:[g rootGroup] error:&err];
        PASS([back isEqual:cfg], "TTIOInstrumentConfig round-trips");
    ...
    PASS([run count] == 1000, "TTIOIndexable count is 1000");
    ...
    PASS(back.acquisitionMode == TTIOAcquisitionModeMS1DDA, "acquisition mode round-trips");
    ...
    for (int p = 0; p < 3; p++) {
        ...
        NSError *e = nil;
        TTIOMassSpectrum *s = [back spectrumAtIndex:pick error:&e];
        PASS(s != nil, "random-access spectrum read");
        TTIOMassSpectrum *expected = spectra[pick];
    ...
    {
        TTIOValueRange *r = [TTIOValueRange rangeWithMinimum:10.0 maximum:12.0];
    ...
    // ---- streaming iteration ----
    {
        ...
        while ([back hasMore]) {
            TTIOMassSpectrum *s = [back nextObject];
            if (!s) break;
    ...
    {
        NSError *e = nil;
        TTIOMassSpectrum *s = [back spectrumAtIndex:1500 error:&e];
        PASS(s == nil, "out-of-range index returns nil");
        PASS(e != nil && e.code == TTIOErrorOutOfRange,
