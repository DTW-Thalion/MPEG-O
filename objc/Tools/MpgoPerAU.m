/*
 * MpgoPerAU — v1.0 cross-language conformance CLI for per-AU
 *             encryption. Parallel to Python
 *             mpeg_o.tools.per_au_cli and Java
 ...
 *
 * Usage:
 *   MpgoPerAU encrypt  in.mpgo out.mpgo keyfile [--headers]
 *   MpgoPerAU decrypt  in.mpgo out.mpad keyfile
 *   MpgoPerAU send     in.mpgo out.mots
 *   MpgoPerAU recv     in.mots out.mpgo
 *
...
static NSString *headersJson(NSArray *rows)
{
    ...
    for (NSUInteger i = 0; i < rows.count; i++) {
        ...
        MPGOAUHeaderPlaintext *r = rows[i];
        [out appendString:@"{"];
...
static int cmdEncrypt(int argc, const char **argv)
{
    ...
    NSError *err = nil;
    BOOL ok = [MPGOPerAUFile encryptFilePath:out
...
static int cmdDecrypt(int argc, const char **argv)
{
    ...
    NSString *keyPath = @(argv[4]);
    NSError *err = nil;
    NSDictionary *plain = [MPGOPerAUFile decryptFilePath:in
                                                      key:readKeyFile(keyPath)
                                             providerName:nil
                                                    error:&err];
    if (!plain) {
...
static int cmdSend(int argc, const char **argv)
{
    ...
    NSString *in = @(argv[2]);
    NSString *out = @(argv[3]);
    MPGOTransportWriter *tw = [[MPGOTransportWriter alloc] initWithOutputPath:out];
    NSError *err = nil;
    BOOL ok = [MPGOEncryptedTransport writeEncryptedDataset:in
                                                       writer:tw
                                                 providerName:nil
                                                        error:&err];
    [tw close];
...
static int cmdRecv(int argc, const char **argv)
{
    ...
    NSError *err = nil;
    BOOL ok = [MPGOEncryptedTransport readEncryptedToPath:out
                                                fromStream:stream
                                               providerName:nil
                                                      error:&err];
    if (!ok) {
...
int main(int argc, const char **argv)
{
    @autoreleasepool {
        if (argc < 2) {
            fprintf(stderr, "usage: MpgoPerAU <encrypt|decrypt|send|recv> ...\n");
            return 2;
        }
