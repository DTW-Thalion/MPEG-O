/*
 * MpgoSimulator — v0.10 M69 command-line tool.
 *
 * Usage:
 *   MpgoSimulator <output.mots> [--scan-rate N] [--duration N]
 *                 [--ms1-fraction N] [--mz-min N] [--mz-max N]
 *                 [--n-peaks N] [--seed N]
 *
 * Generates a synthetic MPEG-O transport stream. Parallel to Python
 * ``python -m mpeg_o.tools.simulator_cli`` and Java
 * ``com.dtwthalion.mpgo.tools.SimulatorCli``.
 *
...
int main(int argc, const char **argv)
{
    @autoreleasepool {
        if (argc < 2) {
            fprintf(stderr, "usage: MpgoSimulator <output.mots> "
        ...
        MPGOTransportWriter *tw =
            [[MPGOTransportWriter alloc] initWithOutputPath:output];
        NSError *err = nil;
