/*
 * TtioDumpIdentifications — v0.7 M51 compound parity dumper.
 *
 * Reads an .tio file and emits the dataset-wide identifications,
 * quantifications, and provenance records as deterministic JSON to
 * stdout. Byte-identical to the Python
 * ``ttio.tools.dump_identifications`` module and the Java
 * ``DumpIdentifications`` class.
 *
 * Build via gnustep-make (see GNUmakefile).
 * Usage: TtioDumpIdentifications <path-to-.tio>
 *
 * Exit codes:
...
static NSString *formatValue(id v)
{
    ...
    if ([v isKindOfClass:[NSNumber class]]) {
        ...
        // TTIO compound records don't carry booleans today; encode as
        // integer to match the Python fallback.
        return formatInt(n.longLongValue);
    }
    [NSException raise:@"TTIOCanonicalJSONError"
...
static NSDictionary *identificationRecord(TTIOIdentification *i)
{
    return @{
...
static NSDictionary *quantificationRecord(TTIOQuantification *q)
{
    return @{
...
static NSDictionary *provenanceRecord(TTIOProvenanceRecord *p)
{
...
int main(int argc, const char *argv[])
{
    @autoreleasepool {
        ...
        TTIOSpectralDataset *ds =
            [TTIOSpectralDataset readFromFilePath:path error:&error];
        if (ds == nil) {
        ...
        for (TTIOIdentification *i in ds.identifications) {
            [idents addObject:identificationRecord(i)];
        }
        NSMutableArray *quants = [NSMutableArray array];
        for (TTIOQuantification *q in ds.quantifications) {
