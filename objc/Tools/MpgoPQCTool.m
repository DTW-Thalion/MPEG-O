/*
 * MpgoPQCTool — cross-language PQC conformance CLI (v0.8 M54).
 *
 * Invoked by the cross-language conformance harness
 * (python/tests/test_m54_pqc_conformance.py) as the ObjC counterpart
 * of the Java com.dtwthalion.mpgo.tools.PQCTool. Subcommands match the
...
static int subKemEncaps(int argc, char **argv)
{
    ...
    MPGOPQCKemEncapResult *r =
        [MPGOPostQuantumCrypto kemEncapsulateWithPublicKey:pk error:&err];
    if (!r) {
...
static int subKemDecaps(int argc, char **argv)
{
    ...
    NSData *ss = [MPGOPostQuantumCrypto kemDecapsulateWithPrivateKey:sk
                                                           ciphertext:ct
                                                                error:&err];
    if (!ss) {
...
static int subHdf5Sign(int argc, char **argv)
{
    ...
    BOOL ok = [MPGOSignatureManager signDataset:@(argv[3])
                                          inFile:@(argv[2])
                                         withKey:sk
                                       algorithm:@"ml-dsa-87"
                                           error:&err];
...
static int subHdf5Verify(int argc, char **argv)
{
    ...
    BOOL ok = [MPGOSignatureManager verifyDataset:@(argv[3])
                                            inFile:@(argv[2])
                                           withKey:pk
                                         algorithm:@"ml-dsa-87"
                                             error:&err];
...
int main(int argc, char **argv)
{
    @autoreleasepool {
        if (argc < 2) {
            fprintf(stderr,
                "usage: MpgoPQCTool <subcommand> [args...]\n"
                "  sig-keygen  PK_OUT  SK_OUT\n"
                "  sig-sign    SK_IN   MSG_IN  SIG_OUT\n"
