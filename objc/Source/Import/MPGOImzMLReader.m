...
#import "MPGOImzMLReader.h"

NSString *const MPGOImzMLReaderErrorDomain = @"MPGOImzMLReaderErrorDomain";

#pragma mark - Pixel spectrum

@implementation MPGOImzMLPixelSpectrum
{
    NSData *_mzArray;
...
@implementation MPGOImzMLImport
...
- (instancetype)initWithMode:(NSString *)mode
                 ...
                 scanPattern:(NSString *)scanPattern
                     spectra:(NSArray<MPGOImzMLPixelSpectrum *> *)spectra
...
@interface MPGOImzMLReaderState : NSObject
...
@implementation MPGOImzMLReaderState
- (instancetype)init {
    if ((self = [super init])) {
        _mode = @"";
        _uuidHex = @"";
...
@interface MPGOImzMLReader () <NSXMLParserDelegate>
@end

@implementation MPGOImzMLReader
{
    MPGOImzMLReaderState *_state;
...
+ (NSError *)errorWithCode:(MPGOImzMLReaderErrorCode)code message:(NSString *)message {
    return [NSError errorWithDomain:MPGOImzMLReaderErrorDomain
                               code:code
                                           ...
                                           error:(NSError **)error
{
    ...
    if (![fm fileExistsAtPath:imzmlPath]) {
        if (error) *error = [self errorWithCode:MPGOImzMLReaderErrorMissingFile
    ...
    if (!xmlData) {
        if (error) *error = [self errorWithCode:MPGOImzMLReaderErrorParseFailed
                                        message:[NSString stringWithFormat:@"cannot read imzML: %@", imzmlPath]];
        return nil;
    }

    MPGOImzMLReader *reader = [[MPGOImzMLReader alloc] init];
    reader->_state = [[MPGOImzMLReaderState alloc] init];
    ...
    if (reader->_state.mode.length == 0) {
        if (error) *error = [self errorWithCode:MPGOImzMLReaderErrorMissingMetadata
    ...
    if (ibdData.length < 16) {
        if (error) *error = [self errorWithCode:MPGOImzMLReaderErrorBinaryShorterThanUUID
    ...
    if (![ibdUUIDHex isEqualToString:reader->_state.uuidHex]) {
        if (error) *error = [self errorWithCode:MPGOImzMLReaderErrorUUIDMismatch
    ...
    NSArray<MPGOImzMLPixelSpectrum *> *pixels =
    ...
    return [[MPGOImzMLImport alloc] initWithMode:reader->_state.mode
...
- (NSArray<MPGOImzMLPixelSpectrum *> *)materialiseSpectraWithIBD:(NSData *)ibdData
                                                          ibdPath:(NSString *)ibdPath
                                                            error:(NSError **)error
{
    NSMutableArray<MPGOImzMLPixelSpectrum *> *pixels = [NSMutableArray array];
    NSData *sharedMz = nil;
    ...
    for (NSDictionary *stub in _state.stubs) {
        ...
        if (effectiveMz.length / sizeof(double) != intData.length / sizeof(double)) {
            if (error) *error = [[self class] errorWithCode:MPGOImzMLReaderErrorOffsetOverflow
                                                    message:[NSString stringWithFormat:@"%@: mz/intensity size mismatch", ibdPath]];
            return nil;
        }
        MPGOImzMLPixelSpectrum *pixel = [[MPGOImzMLPixelSpectrum alloc] initWithX:[stub[@"x"] integerValue]
