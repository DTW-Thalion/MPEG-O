...
const uint8_t MPGOTransportHeaderMagic[2] = {'M', 'O'};
const uint8_t MPGOTransportVersion = 0x01;
const NSUInteger MPGOTransportHeaderSize = 24;

NSString *const MPGOTransportErrorDomain = @"MPGOTransportErrorDomain";

// ---------------------------------------------------------------- CRC-32C

static uint32_t MPGOTransportCRC32CTable[256];
static void MPGOTransportCRC32CBuildTable(void)
{
    ...
    dispatch_once(&once, ^{
        ...
        for (int b = 0; b < 256; b++) {
            ...
            MPGOTransportCRC32CTable[b] = crc;
        }
    });
}
...
{
    MPGOTransportCRC32CBuildTable();
    uint32_t crc = 0xFFFFFFFFu;
    for (NSUInteger i = 0; i < length; i++) {
        crc = (crc >> 8) ^ MPGOTransportCRC32CTable[(crc ^ data[i]) & 0xFFu];
    }
...
@implementation MPGOTransportPacketHeader
...
{
    uint8_t buf[24];
    buf[0] = MPGOTransportHeaderMagic[0];
    buf[1] = MPGOTransportHeaderMagic[1];
    buf[2] = MPGOTransportVersion;
...
+ (instancetype)decodeFromBytes:(const uint8_t *)bytes
                          length:(NSUInteger)length
                           error:(NSError **)error
{
    if (length < MPGOTransportHeaderSize) {
        if (error) *error = [NSError errorWithDomain:MPGOTransportErrorDomain
                             ...
                             [NSString stringWithFormat:@"header needs %lu bytes, got %lu",
                                 (unsigned long)MPGOTransportHeaderSize, (unsigned long)length]}];
        return nil;
    }
    if (bytes[0] != 'M' || bytes[1] != 'O') {
        if (error) *error = [NSError errorWithDomain:MPGOTransportErrorDomain
                                                 code:MPGOTransportErrorBadMagic
                                             userInfo:@{NSLocalizedDescriptionKey:
                             @"invalid packet magic"}];
        return nil;
    }
    if (bytes[2] != MPGOTransportVersion) {
        if (error) *error = [NSError errorWithDomain:MPGOTransportErrorDomain
                                                 code:MPGOTransportErrorBadVersion
                                             userInfo:@{NSLocalizedDescriptionKey:
                             [NSString stringWithFormat:@"unsupported transport version: %u",
                                 (unsigned)bytes[2]]}];
        return nil;
    }
    return [[self alloc] initWithPacketType:(MPGOTransportPacketType)bytes[3]
