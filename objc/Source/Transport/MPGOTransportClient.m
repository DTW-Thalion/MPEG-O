...
@interface MPGOTransportClient ()
{
    NSString *_url;
    NSString *_queryJSON;
    NSMutableData *_pendingFrame;  // accumulates fragmented frames
    NSMutableArray<MPGOTransportPacketRecord *> *_packets;
                        ...
                        void *user, void *in, size_t len)
{
    ...
    MPGOTransportClient *client = opaque ? (__bridge MPGOTransportClient *)opaque : nil;
    if (!client) return 0;
...
@implementation MPGOTransportClient
...
- (NSArray<MPGOTransportPacketRecord *> *)
                    ...
                    error:(NSError **)error
{
    ...
    // MPGOTransportReader for materialization.
    NSMutableData *buf = [NSMutableData data];
    for (MPGOTransportPacketRecord *rec in packets) {
        [buf appendData:[rec.header encode]];
        [buf appendData:rec.payload];
        if (rec.header.flags & MPGOTransportPacketFlagHasChecksum) {
            uint32_t crc = MPGOTransportCRC32C((const uint8_t *)rec.payload.bytes,
                                                 rec.payload.length);
            uint8_t crcBuf[4];
    ...
    MPGOTransportReader *reader = [[MPGOTransportReader alloc] initWithData:buf];
    return [reader writeMpgoToPath:outputPath error:error];
}
...
{
    if (raw.length < MPGOTransportHeaderSize) {
        _protocolError = [NSError errorWithDomain:MPGOTransportErrorDomain
                                               code:MPGOTransportErrorTruncated
                                           userInfo:@{NSLocalizedDescriptionKey:
    ...
    NSError *err = nil;
    MPGOTransportPacketHeader *header =
        [MPGOTransportPacketHeader decodeFromBytes:bytes length:raw.length error:&err];
    if (!header) {
        _protocolError = err;
        _connectionClosed = YES;
        return;
    }
    NSUInteger consumed = MPGOTransportHeaderSize + header.payloadLength;
    ...
    NSData *payload = [NSData dataWithBytes:bytes + MPGOTransportHeaderSize
                                      length:header.payloadLength];

    if (header.flags & MPGOTransportPacketFlagHasChecksum) {
    ...
    MPGOTransportPacketRecord *rec =
        [[MPGOTransportPacketRecord alloc] initWithHeader:header payload:payload];
    [_packets addObject:rec];
    if (header.packetType == MPGOTransportPacketEndOfStream) {
        _sawEndOfStream = YES;
    }
}
