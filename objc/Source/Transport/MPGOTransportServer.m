...
@interface MPGOServerSession : NSObject
@property (nonatomic, strong) NSMutableArray<NSData *> *queue;
@property (nonatomic, weak) MPGOTransportServer *server;  // back-pointer for dataset access
@end
@implementation MPGOServerSession
- (instancetype)init {
...
@interface MPGOTransportServer ()
- (MPGOSpectralDataset *)_openDataset:(NSError **)error;
- (uint16_t)_wireFromPolarity:(MPGOPolarity)p;
...
static NSData *frameForPacket(MPGOTransportPacketType type,
                                uint16_t datasetId,
                                uint32_t auSequence,
                                NSData *payload)
{
...
static NSString *instrumentJSON(MPGOInstrumentConfig *cfg)
{
    if (!cfg) return @"{}";
    NSDictionary *d = @{
...
static void buildStreamFrames(MPGOTransportServer *server,
                                MPGOAUFilter *filter,
                                NSMutableArray<NSData *> *outFrames)
{
    NSError *err = nil;
    MPGOSpectralDataset *dataset = [server _openDataset:&err];
    if (!dataset) return;
    ...
    // StreamHeader
    {
        ...
        [outFrames addObject:frameForPacket(MPGOTransportPacketStreamHeader, 0, 0, p)];
    }
    ...
    for (NSString *name in runNames) {
        ...
        appendLEString(p, run.spectrumClassName ?: @"MPGOMassSpectrum", 2);
        ...
        [outFrames addObject:frameForPacket(MPGOTransportPacketDatasetHeader, did, 0, p)];
        did++;
    }
    ...
    for (NSString *name in runNames) {
        ...
        for (NSUInteger i = 0; i < count; i++) {
            ...
            if ([sp isKindOfClass:[MPGOMassSpectrum class]]) {
                MPGOMassSpectrum *ms = (MPGOMassSpectrum *)sp;
            ...
            for (NSString *cname in channelNames) {
                MPGOSignalArray *sa = sp.signalArrays[cname];
                if (!sa) continue;
            ...
            [outFrames addObject:frameForPacket(
                MPGOTransportPacketAccessUnit, did, (uint32_t)i, [au encode])];
            emitted++;
        }
        did++;
    }
    ...
    for (NSString *name in runNames) {
        ...
        appendU16LE(p, did);
        appendU32LE(p, (uint32_t)[run count]);
        [outFrames addObject:frameForPacket(MPGOTransportPacketEndOfDataset, did, 0, p)];
        did++;
    }

    // EndOfStream
    [outFrames addObject:frameForPacket(MPGOTransportPacketEndOfStream, 0, 0, [NSData data])];
}
                            ...
                            void *user, void *in, size_t len)
{
    ...
    switch (reason) {
        case LWS_CALLBACK_ESTABLISHED: {
            ...
            MPGOTransportServer *server =
                (__bridge MPGOTransportServer *)lws_context_user(ctx);
            MPGOServerSession *session = [[MPGOServerSession alloc] init];
        ...
        case LWS_CALLBACK_RECEIVE: {
            ...
            if (!opaque) return 0;
            MPGOServerSession *session = (__bridge MPGOServerSession *)opaque;
            NSString *text = [[NSString alloc]
                initWithBytes:in length:len encoding:NSUTF8StringEncoding];
            MPGOAUFilter *filter = [MPGOAUFilter filterFromQueryJSON:(text ?: @"")];
        ...
        case LWS_CALLBACK_CLOSED: {
            void *opaque = lws_get_opaque_user_data(wsi);
            if (opaque) {
                MPGOServerSession *session =
                    (__bridge_transfer MPGOServerSession *)opaque;
...
@implementation MPGOTransportServer
{
...
- (MPGOSpectralDataset *)_openDataset:(NSError **)error
{
    return [MPGOSpectralDataset readFromFilePath:_datasetPath error:error];
}
...
{
    MPGOTransportServer *self = (__bridge MPGOTransportServer *)arg;
    while (self->_running) {
