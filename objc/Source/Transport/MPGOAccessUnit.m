...
@implementation MPGOTransportChannelData
...
- (NSData *)encode
{
    ...
    for (MPGOTransportChannelData *ch in _channels) {
                           ...
                           error:(NSError **)error
{
    ...
    NSMutableArray<MPGOTransportChannelData *> *channels =
        [NSMutableArray arrayWithCapacity:nChannels];
    for (uint8_t i = 0; i < nChannels; i++) {
        MPGOTransportChannelData *ch =
            [MPGOTransportChannelData decodeFromBytes:bytes
                                                length:length
                                                offset:&offset];
        if (!ch) {
            if (error) *error = [NSError errorWithDomain:MPGOTransportErrorDomain
