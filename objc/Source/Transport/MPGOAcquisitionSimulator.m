 ...
 */
#import "MPGOAcquisitionSimulator.h"
...
@implementation MPGOAcquisitionSimulator
{
...
- (MPGOAccessUnit *)_generateAUForIndex:(NSUInteger)i
                            lastMs1Peak:(double *)lastMs1PeakInOut
{
            ...
            initWithName:@"mz"
             ...
             compression:MPGOCompressionNone
               nElements:(uint32_t)n
                    data:mzData];
    MPGOTransportChannelData *chInt =
        [[MPGOTransportChannelData alloc]
    ...
    return [[MPGOAccessUnit alloc]
               initWithSpectrumClass:0
                     acquisitionMode:(uint8_t)MPGOAcquisitionModeMS1DDA
                             msLevel:msLevel
                            polarity:0
                       retentionTime:rt
...
static NSString *const kInstrumentJSON =
    ...
    @"\"manufacturer\": \"MPEG-O simulator\", \"model\": \"synthetic-v1\", "
    @"\"serial_number\": \"\", \"source_type\": \"\"}";

- (NSUInteger)streamToWriter:(MPGOTransportWriter *)writer
                        error:(NSError **)error
{
    [self _seedRNG];
    ...
    if (![writer writeDatasetHeaderWithDatasetId:1
                                             name:@"simulated_run"
                                  acquisitionMode:(uint8_t)MPGOAcquisitionModeMS1DDA
                                    spectrumClass:@"MPGOMassSpectrum"
    ...
    for (NSUInteger i = 0; i < count; i++) {
        MPGOAccessUnit *au = [self _generateAUForIndex:i lastMs1Peak:&lastMs1Peak];
...
- (NSUInteger)streamPacedToWriter:(MPGOTransportWriter *)writer
                             error:(NSError **)error
{
    [self _seedRNG];
    NSUInteger count = [self scanCount];
