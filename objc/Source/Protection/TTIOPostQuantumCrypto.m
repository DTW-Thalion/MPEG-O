...
#if TTIO_HAVE_LIBOQS
#  include <oqs/oqs.h>
#  define TTIO_PQC_ENABLED 1
#else
#  define TTIO_PQC_ENABLED 0
#endif

@implementation TTIOPQCKeyPair
...
@implementation TTIOPQCKemEncapResult
...
static BOOL TTIO_SetUnavailableError(NSError **error)
{
    if (error) {
        *error = TTIOMakeError(TTIOErrorInvalidArgument,
            @"libTTIO was built without liboqs — PQC entry points "
            @"are not callable. Install liboqs (pip install "
            @"'ttio[pqc]' auto-installs to $HOME/_oqs) and rebuild.");
    }
    return NO;
}

@implementation TTIOPostQuantumCrypto

+ (BOOL)isAvailable
{
    return TTIO_PQC_ENABLED;
}
...
+ (nullable TTIOPQCKeyPair *)kemKeygenWithError:(NSError **)error
{
    ...
    if (!kem) {
        if (error) *error = TTIOMakeError(TTIOErrorInvalidArgument,
            @"liboqs: ML-KEM-1024 is not enabled in this libTTIO build");
        return nil;
    }
    ...
    return [[TTIOPQCKeyPair alloc] initWithPublicKey:pk privateKey:sk];
}
...
{
    OQS_KEM *kem = OQS_KEM_new(OQS_KEM_alg_ml_kem_1024);
    if (!kem) return (TTIOPQCKemEncapResult *)(TTIO_SetUnavailableError(error), nil);
    ...
    return [[TTIOPQCKemEncapResult alloc] initWithCiphertext:ct sharedSecret:ss];
}
...
{
    OQS_KEM *kem = OQS_KEM_new(OQS_KEM_alg_ml_kem_1024);
    if (!kem) { TTIO_SetUnavailableError(error); return nil; }
...
+ (nullable TTIOPQCKeyPair *)sigKeygenWithError:(NSError **)error
{
    ...
    if (!sig) {
        if (error) *error = TTIOMakeError(TTIOErrorInvalidArgument,
            @"liboqs: ML-DSA-87 is not enabled in this libTTIO build");
        return nil;
    }
...
{
    OQS_SIG *sig = OQS_SIG_new(OQS_SIG_alg_ml_dsa_87);
    if (!sig) { TTIO_SetUnavailableError(error); return nil; }
...
{
    OQS_SIG *sig = OQS_SIG_new(OQS_SIG_alg_ml_dsa_87);
    if (!sig) return TTIO_SetUnavailableError(error);
...
#else  // TTIO_PQC_ENABLED == 0 — liboqs absent at build time

+ (nullable TTIOPQCKeyPair *)kemKeygenWithError:(NSError **)error
    { TTIO_SetUnavailableError(error); return nil; }
+ (nullable TTIOPQCKemEncapResult *)kemEncapsulateWithPublicKey:(NSData *)publicKey
                                                            error:(NSError **)error
    { TTIO_SetUnavailableError(error); return nil; }
+ (nullable NSData *)kemDecapsulateWithPrivateKey:(NSData *)privateKey
...
+ (BOOL)sigVerifyWithPublicKey:(NSData *)publicKey
                       ...
                       signature:(NSData *)signature
                           error:(NSError **)error
    { return TTIO_SetUnavailableError(error); }

#endif /* TTIO_PQC_ENABLED */
