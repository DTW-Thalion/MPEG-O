#ifndef MPGO_SIGNATURE_MANAGER_H
#define MPGO_SIGNATURE_MANAGER_H

#import <Foundation/Foundation.h>

/**
 * HMAC-SHA256 signing and verification for MPEG-O datasets. Signatures
 * are stored on the target HDF5 dataset as a base64-encoded string
 * attribute named @mpgo_signature. A file that contains any signed
 * datasets also carries opt_digital_signatures in @mpeg_o_features.
 *
 ...
 *  the .mpgo file at `filePath`. Stores the HMAC as a base64 string
 *  attribute `@mpgo_signature` on that dataset and writes
 ...
 *  `@mpgo_signature` attribute. Returns YES if they match. On
 *  mismatch returns NO with a descriptive NSError in the MPGOError
 *  domain. A missing attribute returns NO with
 *  MPGOErrorAttributeRead — use MPGOVerifier for the three-state
 *  view. */
