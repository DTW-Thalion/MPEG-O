                                ...
                                error:(NSError **)error
{
    if (m < 1) {
        if (error) *error = MPGOMakeError(MPGOErrorInvalidArgument,
            @"MPGOTwoDCos: m must be >= 1, got %lu", (unsigned long)m);
        return nil;
    }
...
+ (MPGOTwoDimensionalCorrelationSpectrum *)computeWithDynamicSpectra:(NSData *)dynamicSpectra
                                                    perturbationPoints:(NSUInteger)m
                                                     spectralVariables:(NSUInteger)n
                                                             reference:(NSData *)reference
                                                          variableAxis:(MPGOAxisDescriptor *)variableAxis
                                                                 ...
                                                                 error:(NSError **)error
{
    if (dynamicSpectra == nil) {
        if (error) *error = MPGOMakeError(MPGOErrorInvalidArgument,
            @"MPGOTwoDCos: dynamicSpectra must not be nil");
        return nil;
    }
    if (m < 2) {
        if (error) *error = MPGOMakeError(MPGOErrorInvalidArgument,
            @"MPGOTwoDCos: need >= 2 perturbation points, got m=%lu",
            (unsigned long)m);
        return nil;
    }
    if (n < 1) {
        if (error) *error = MPGOMakeError(MPGOErrorInvalidArgument,
            @"MPGOTwoDCos: n must be >= 1, got %lu", (unsigned long)n);
        return nil;
    }
    NSUInteger expected = (NSUInteger)(m * n * sizeof(double));
    if (dynamicSpectra.length != expected) {
        if (error) *error = MPGOMakeError(MPGOErrorInvalidArgument,
            @"MPGOTwoDCos: dynamicSpectra bytes %lu != m*n*8 = %lu",
    ...
    if (reference != nil && reference.length != refExpected) {
        if (error) *error = MPGOMakeError(MPGOErrorInvalidArgument,
            @"MPGOTwoDCos: reference bytes %lu != n*8 = %lu",
                                          ...
                                          error:(NSError **)error
{
    if (synchronous == nil || asynchronous == nil) {
        if (error) *error = MPGOMakeError(MPGOErrorInvalidArgument,
            @"MPGOTwoDCos: matrices must not be nil");
        return nil;
    }
    if (synchronous.length != asynchronous.length) {
        if (error) *error = MPGOMakeError(MPGOErrorInvalidArgument,
            @"MPGOTwoDCos: shape mismatch %lu vs %lu",
