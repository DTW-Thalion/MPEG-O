...
+ (instancetype)readFromFilePath:(NSString *)path error:(NSError **)error
{
    ...
    // no /study group, bypass the MPGOSpectralDataset reader entirely
    ...
    if (probe >= 0) {
        ...
        if (!hasStudy && hasRootImage) {
            ...
            mpgo_image_meta_t meta; memset(&meta, 0, sizeof(meta));
            ...
            H5Fclose(probe);
            if (!cube) return nil;
            MPGOMSImage *img = [[MPGOMSImage alloc]
    ...
    return (MPGOMSImage *)[super readFromFilePath:path error:error];
}
