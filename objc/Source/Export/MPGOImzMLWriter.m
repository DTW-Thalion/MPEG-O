 ...
 */
#import "MPGOImzMLWriter.h"
#import "Import/MPGOImzMLReader.h"
#import <openssl/sha.h>


@implementation MPGOImzMLWriteResult {
    NSString *_imzmlPath;
    NSString *_ibdPath;
    NSString *_uuidHex;
    NSString *_mode;
...
static NSError *MakeError(NSInteger code, NSString *fmt, ...) {
    ...
    va_end(ap);
    return [NSError errorWithDomain:@"MPGOImzMLWriter" code:code
                             userInfo:@{NSLocalizedDescriptionKey: msg}];
}


@implementation MPGOImzMLWriter

+ (nullable MPGOImzMLWriteResult *)writePixels:(NSArray<MPGOImzMLPixelSpectrum *> *)pixels
                                         ...
                                         error:(NSError **)error
{
    ...
    if (gridMaxX == 0) {
        NSInteger m = 0;
        for (MPGOImzMLPixelSpectrum *p in pixels) if (p.x > m) m = p.x;
        gridMaxX = m;
    }
    if (gridMaxY == 0) {
        NSInteger m = 0;
        for (MPGOImzMLPixelSpectrum *p in pixels) if (p.y > m) m = p.y;
        gridMaxY = m;
    }
    ...
    if ([mode isEqualToString:@"continuous"]) {
        ...
        for (NSUInteger i = 0; i < pixels.count; i++) {
            MPGOImzMLPixelSpectrum *p = pixels[i];
        ...
        for (MPGOImzMLPixelSpectrum *p in pixels) {
    ...
    [xml appendString:@"    <software id=\"mpeg_o\" version=\"0.9.0\">\n"];
    [xml appendString:@"      <cvParam cvRef=\"MS\" accession=\"MS:1000799\""
                      @" name=\"custom unreleased software tool\" value=\"mpeg-o\"/>\n"];
    ...
    [xml appendString:@"      <processingMethod order=\"0\" softwareRef=\"mpeg_o\">\n"];
    ...
    [xml appendString:@"  <run id=\"mpgo_imzml_export\" defaultInstrumentConfigurationRef=\"IC1\">\n"];
    ...
    return [[MPGOImzMLWriteResult alloc] initInternal:imzmlPath ibd:ibd
                                                  uuid:uuid mode:mode
                                               nPixels:pixels.count];
}

+ (nullable MPGOImzMLWriteResult *)writeFromImport:(MPGOImzMLImport *)import
                                         toImzMLPath:(NSString *)imzmlPath
                                             ibdPath:(nullable NSString *)ibdPath
                                               error:(NSError **)error
{
