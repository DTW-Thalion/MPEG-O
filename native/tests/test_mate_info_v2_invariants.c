...
static void test_i7_reserved_mf_raw_pack_rejection(void) {
    ...
    /* For N=1 with all-NO_MATE, raw-pack will be smaller than zlib
     * (1+1=2 bytes vs larger zlib framing). The MF substream
    ...
    if (encoded[34] == MIV2_MF_RAW_PACK) {
        encoded[35] = (encoded[35] & 0xFC) | 0x03;  /* set low 2 bits to 3 */
        ...
        /* If encoder picked zlib path, skip this branch -- zlib path
         * with synthetic value 3 is harder to construct here; T4
         * stress test exercises both paths organically. */
        printf("I7 reserved MF (raw-pack) rejection: SKIP (encoder picked zlib path for N=1)\n");
    }
}

/* I8: encoder rejects mate_chrom_id < -1 (which would leak to MF=3 by
 * extension). The classifier defines: -1 -> MF=2, >=0 -> MF=0 or 1.
 * Values < -1 are invalid input. */
