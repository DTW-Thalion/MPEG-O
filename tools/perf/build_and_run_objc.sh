...
# built libMPGO. If BUILD_PG=1 is set, also rebuilds libMPGO with -pg
...
# Compile the harness with libMPGO's headers on the include path.
# libMPGO is ARC; the harness itself can be ARC too.
