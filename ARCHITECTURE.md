...
| `TTIOAcquisitionRun` | `NSObject` | `TTIOIndexable`, `TTIOStreamable` | `spectrumIndex`, `instrumentConfig`, `acquisitionMode`; lazy reads when read from disk: `channelRange:start:count:error:` slices a channel (block-wise for codec 17), `iterSpectraWithBatch:usingBlock:` walks the run with bounded memory |
| `TTIOGenomicRun` | `NSObject` | `TTIOIndexable`, `TTIOStreamable` | `genomicIndex`, `referenceUri`, `platform`, `sampleName`, `acquisitionMode`; element type is `TTIOAlignedRead`, not `TTIOSpectrum`. Reads the whole-channel layout and `blocks_v1` (`layout`, `blockCount`, `chromosomeNames`, `iterReadsFrom:to:usingBlock:`); under `blocks_v1` one block is materialised at a time as a memory-provider group (`TTIOBlockTable`, `TTIOBlockView`) and the last one cached |
| `TTIOGenomicStreamWriter` | `NSObject` | — | Writes a genomic run as `blocks_v1` from batches (`appendRead:`, `appendBatch:`, `flush:`, `close:`); `TTIOGenomicStreamWriterOptions` carries the run metadata and block policy. `TTIOGenomicBlocks` slices, concatenates and encodes blocks; `TTIOLazyReference` is an `NSDictionary` over an indexed FASTA |
| `TTIOSpectralStreamWriter` | `NSObject` | — | Writes a spectral run from `TTIOWrittenSpectralBatch` batches or single spectra into extendable datasets, finalising the codec-17 header at close |
| `TTIOGenomicStreamSource` / `TTIOSpectralStreamSource` | `NSObject` | — | A run delivered as a batch producer block; `TTIOImportedDataset.genomicStreams` / `spectralStreams` write them after the static content |
