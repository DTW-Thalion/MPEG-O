...
public class Hdf5Group implements AutoCloseable {
    ...
    /**
     * Write a scalar IEEE-754 double attribute.  Replaces any existing
     * attribute of the same name regardless of its previous type.
     */
    ...
    /**
     * Read a scalar double attribute.  Returns {@code defaultValue} if the
     * attribute is absent.
     */
    public double readDoubleAttribute(String name, double defaultValue) {
        ...
        long aid = -1;
        try {
            if (!H5.H5Aexists(groupId, name)) return defaultValue;
            aid = H5.H5Aopen(groupId, name, HDF5Constants.H5P_DEFAULT);
