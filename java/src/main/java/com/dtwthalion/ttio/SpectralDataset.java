...
import com.dtwthalion.ttio.hdf5.Hdf5CompoundIO;
import com.dtwthalion.ttio.hdf5.Hdf5File;
import com.dtwthalion.ttio.hdf5.Hdf5Group;
...
public class SpectralDataset implements
        com.dtwthalion.ttio.protocols.Encryptable,
        AutoCloseable {
    ...
    private com.dtwthalion.ttio.protection.AccessPolicy accessPolicy;
    ...
    private static SpectralDataset openViaProvider(String url) {
        StorageProvider provider = com.dtwthalion.ttio.providers
                .ProviderRegistry.open(url, StorageProvider.Mode.READ);
        try (com.dtwthalion.ttio.providers.StorageGroup root =
                provider.rootGroup()) {
            FeatureFlags flags = FeatureFlags.readFrom(root);
            String encryptedAlg = "";
            ...
            if (root.hasChild("study")) {
                try (com.dtwthalion.ttio.providers.StorageGroup study =
                    ...
                    if (study.hasChild("ms_runs")) {
                        try (com.dtwthalion.ttio.providers.StorageGroup ms =
    ...
    private static List<Identification> readIdentificationsFromJson(
            com.dtwthalion.ttio.providers.StorageGroup study) {
    ...
    @Override
    public void encryptWithKey(byte[] key, com.dtwthalion.ttio.Enums.EncryptionLevel level)
            throws Exception {
    ...
    private void markRootEncrypted() {
        ...
        if (path != null) {
            try (com.dtwthalion.ttio.hdf5.Hdf5File f =
                         com.dtwthalion.ttio.hdf5.Hdf5File.open(path);
     ...
     * com.dtwthalion.ttio.Enums.EncryptionLevel)} (which leaves the root
    ...
    public static void decryptInPlace(String path, byte[] key) {
        ...
        try (com.dtwthalion.ttio.hdf5.Hdf5File f =
                     com.dtwthalion.ttio.hdf5.Hdf5File.openReadOnly(path);
             Hdf5Group root = f.rootGroup()) {
            if (root.hasChild("study")) {
                try (Hdf5Group study = root.openGroup("study")) {
                    if (study.hasChild("ms_runs")) {
    ...
    public void setAccessPolicy(Object policy) {
        this.accessPolicy = (com.dtwthalion.ttio.protection.AccessPolicy) policy;
    }
