...
| File               | Contents                                                                               |
|--------------------|----------------------------------------------------------------------------------------|
| `minimal_ms.tio`  | 1 MS run, 10 spectra of 8 peaks. No idents/quants/prov. Smallest conformant file.      |
| `full_ms.tio`     | 1 MS run, 12 spectra, 10 compound identifications, 5 compound quantifications, 2 compound provenance records. |
| `nmr_1d.tio`      | 1 NMR run stored under `msRuns` (M10 modality-agnostic), 5 TTIONMRSpectrum of 64 chemical shift points, 1H @ 600.13 MHz. |
| `encrypted.tio`   | Dataset-level AES-256-GCM encrypted: both intensity channel and any compound datasets sealed. Root `@encrypted = "aes-256-gcm"`. |
| `signed.tio`      | HMAC-SHA256 signatures on both `mz_values` and `intensity_values` of `run_0001`. Root feature flag `opt_digital_signatures` present. |
...
1. **`minimal_ms.tio`** — opens without error; reports 10 mass
   spectra in `run_0001`; can read spectrum 5's mz+intensity arrays.
2. **`full_ms.tio`** — reads 10 identifications, 5 quantifications,
   2 provenance records via the compound datasets under `/study/`.
3. **`nmr_1d.tio`** — `run = msRuns["nmr_run"]` materializes as
   an NMR run with `spectrum_class == "TTIONMRSpectrum"`; each
   spectrum has `chemical_shift` and `intensity` channels.
4. **`encrypted.tio`** — root `@encrypted` attribute present;
   `/study/ms_runs/run_0001/signal_channels/intensity_values`
   is absent; `intensity_values_encrypted` dataset is present.
5. **`signed.tio`** — `mz_values` and `intensity_values` carry
   `@ttio_signature` (base64 string); feature list includes
