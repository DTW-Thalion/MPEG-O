...
class AcquisitionSimulatorTest {
    ...
    @Test
    void materializesAsValidTtio(@TempDir Path dir) throws Exception {
        Path mots = dir.resolve("sim.tis");
        try (TransportWriter tw = new TransportWriter(mots)) {
            new AcquisitionSimulator(5.0, 1.0, 0.3, 100.0, 2000.0, 50, 42L)
                    .streamToWriter(tw);
        }
        try (TransportReader tr = new TransportReader(mots);
             com.dtwthalion.ttio.SpectralDataset rt =
                     tr.materializeTo(dir.resolve("sim.tio").toString())) {
            assertEquals("Simulated acquisition", rt.title());
            com.dtwthalion.ttio.AcquisitionRun run =
